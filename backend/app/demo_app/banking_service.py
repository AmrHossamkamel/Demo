...
demo_app = FastAPI(
    title="Botify Demo Banking Microservice",
    description="Dedicated target microservice for operational & security fault injection.",
    version="1.0.0"
)
...
fault_state = {
    "latency_ms": 0,
    "force_500_rate": 0.0, # 0.0 to 1.0 probability
    "force_404_rate": 0.0,
...
def set_fault_injection(config: FaultInjectionConfig):
    ...
    logger.warning(f"Demo Banking Service fault state updated: {fault_state}")
...
async def fault_injection_middleware(request: Request, call_next):
    if fault_state.get("service_stopped") and not request.url.path.startswith("/api/v1/demo"):
        return JSONResponse(
            ...
            content={"error": "Service Unavailable", "message": "Demo Banking Service is currently stopped."}
        )
...
def health():
    return {"status": "HEALTHY", "service": "Demo Banking Core", "timestamp": time.time()}
